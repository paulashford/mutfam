# mutfam_enrichment.R
# Calculates enrichment of mutations in CATH Functional Families (FunFams)
# Based on:
# 	Miller, M.L. et al., 2015.
...
# 12/02/2016: MutFam v2.0 Update to use HGNC_ID and TBL_CANCER_LOOKUP
#             Note there are also many SQL materialised view changes!
...
mf.init <- function() {
  ...
  # ------------------------
  # Run from calling session
  #setwd("/Users/ash/woofgit/fgfr-net-mutations/genome_wide/mutfams/R");
  #source("mutfam_enrichment.R");
  # ------------------------
  source("mutfam_util.R");
...
# mf.main
# ------------------------------------------------------------------------------
mf.main <- function(dbConn, cancer_type, version){
  ...
  # ----------------------
  # mf: muts in funfams
  dt_mf <- mf.calc_mf(dbConn,cancer_type);

  # ng: muts in all containing genes
  dt_ng <- mf.calc_ng(dbConn, cancer_type);
...
# ------------------------------------------------------------------------------
mf.calc_ef <- function (dbConn, ff_num){
	me <- mf.calc_me(dbConn,ff_num);
	if (me >0){
		return(mf.calc_mf(dbConn, ff_num) / me);
	}else {
		return(0);
	}
}
...
# ------------------------------------------------------------------------------
# mf.calc_me(dbConn, ff_num):
# Simple entry function to calculate me by calls to mf.calc_fm and mf.calc_ng.
# ------------------------------------------------------------------------------
mf.calc_me <- function (dbConn, ff_num){
	return(mf.calc_ng(dbConn, ff_num) * mf.calc_fm(dbConn, ff_num));
}


# ------------------------------------------------------------------------------
# mf.calc_mf(dbConn, [ff_num]):
...
#mf.calc_mf <- function (dbConn, sf_num, ff_num, cancer_type){
mf.calc_mf <- function (dbConn, cancer_type){
  require (data.table);
  ...
  sql_where <- dbGetQuery(dbConn, sql_get);
  if (nrow(sql_where) ==1 ){
    if (!is.na(sql_where)){
  ...
  sql_group_by <- dbGetQuery(dbConn, sql_get);
  ...
  dt_mf <- data.table(dbGetQuery(dbConn, sql));
  if (is.na(dt_mf)) {
    return(-1)
  } else
  {
...
#mf.calc_ng <- function (dbConn, ff_num, cancer_type){
mf.calc_ng <- function (dbConn, cancer_type){
  require (data.table);
...
mf.calc_aa_ratio <- function (dbConn){
 ...
 fun_sum <- data.table(dbGetQuery(dbConn,sql_ff));
 if (is.na(fun_sum)) {return(0)};

 gene_sum <- data.table(dbGetQuery(dbConn, sql_gene));
...
mf.get_sf_ff_info <- function (dbConn){
 ...
 sf_ff_info <- data.table(dbGetQuery(dbConn,sql));
 if (is.na(sf_ff_info)) {
   return(0)
   } else {
...
mf.get_sf_info <- function (dbConn){
 ...
 sf_sf_info <- data.table(dbGetQuery(dbConn,sql));
 if (is.na(sf_sf_info)) {
   return(0)
   } else {
